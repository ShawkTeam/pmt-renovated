...
#include "../../../../../../boringssl/src/crypto/internal.h"
#include "../../../../../../boringssl/src/crypto/mem_internal.h"
#include "../../../../../../boringssl/src/crypto/fipsmodule/aes/internal.h"
  ...
  while (!current_range_rest.empty()) {
    ...
    while (in.size() <= BlockSize) {
      ...
      if (collect_from_rest.empty()) {
        ...
        auto finalout = Span<uint8_t>(out).first(in.size());
    ...
    maybe_copy_to_iovec(Span<uint8_t>(out).first(in.size()), current_range_head,
                        current_range_rest);
...
#endif  // #if defined(__clang__) && (__clang_major__ < 11)

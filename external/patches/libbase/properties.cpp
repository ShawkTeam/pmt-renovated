 ...
 *
 * This source file patch enables libbase to be used under API level 30.
 * Needed by liblp.
 */
...
// PATCH: Use old __system_property_read function.
std::string GetProperty(const std::string& key, const std::string& default_value) {
  const prop_info* pi = __system_property_find(key.c_str());
  if (pi == nullptr) return default_value;

  char value[PROP_VALUE_MAX];
  if (__system_property_read(pi, nullptr, value) == 0) return default_value;

  return (value[0] == '\0') ? default_value : std::string(value);
}
...
/*
 * These functions are not required for liblp.

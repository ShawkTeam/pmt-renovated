...
void Builder::scan() {
  if (diskNames.empty()) throw Error("Empty disk path.");
  partitions.clear();
...
void Builder::scanLogicalPartitions() {
  ...
  for (const auto &entry : entries) {
    ...
    if (std::filesystem::is_block_file(final)) logicalPartitions.emplace_back(entry.path());
  }

  std::erase_if(logicalPartitions, [](const LogicalPartition_t &lpart) -> bool {
    return lpart.getName().find("com.") != std::string::npos; // Erase non-partition contents (like
                                                              // com.android.adbd)
  });
}
...
void Builder::findDiskPaths() {
  try {
    ...
    for (const auto &entry : entries) {
      if (std::filesystem::is_block_file(entry.status()) && Extra::isReallyDisk(entry.path().filename()))
  ...
  if (diskNames.size() > 1) isUFS = true;
  if (diskNames.empty()) throw Error("Can't find any disk or partition table in \"/dev/block\"");
...
std::vector<Partition_t> Builder::getPartitionsByDisk(const std::string &name) const {
  if (!diskNames.contains(name)) return {};

  std::vector<Partition_t> parts;
  for (const auto &part : partitions)
    if (part.getDiskName() == name) parts.push_back(part);

  return parts;
}

std::vector<LogicalPartition_t> Builder::getLogicalPartitions() const { return logicalPartitions; }
...
std::vector<std::pair<std::string, uint64_t>> Builder::getDataOfLogicalPartitions() const {
...
std::vector<std::tuple<std::string, uint64_t, bool>> Builder::getDataOfPartitionsByDisk(const std::string &name) {
  std::vector<std::tuple<std::string, uint64_t, bool>> parts;
  for (auto &part : partitions)
    if (part.getDiskName() == name) parts.emplace_back(part.getName(), part.getSize(), part.isSuperPartition());
...
bool Builder::empty() const { return partitions.empty() && logicalPartitions.empty() && gptDataCollection.empty(); }
...
bool Builder::foreachLogicalPartitions(const std::function<bool(LogicalPartition_t &)> &function) {
  bool isSuccess = true;
...
bool Builder::foreachGptData(
    const std::function<bool(const std::filesystem::path &, std::shared_ptr<GPTData> &)> &function) {
  bool isSuccess = true;
...
void Builder::setSeek(const std::string &name) {
  if (diskNames.contains(name)) seek = name;
}

void Builder::setDisks(std::unordered_set<std::string> names) { diskNames = std::move(names); }

void Builder::setGPTDataOf(const std::string &name, std::shared_ptr<GPTData> data) {
  if (auto it = gptDataCollection.find("/dev/block/" + name); it != gptDataCollection.end())
...
bool Builder::operator==(const Builder &other) {
  ...
  return partitions == other.partitions && logicalPartitions == other.logicalPartitions && equal;
}

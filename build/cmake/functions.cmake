...
# Usage: add_shared_super_library(<name> <NO_INTERFACE | USE_INTERFACE | USE_INTERFACE_NOLIBS | USE_INTERFACE_NOLIBS_NOFLAGS> <source(s)> [LIBS <libs> MANUAL_LIBS <libs> INCLUDES <includes> COMPILE_OPTIONS <options> LINKER_OPTIONS <linker-options> LINKER_SCRIPT <linker-script> (CPPSTD <cppstd> OR CSTD <cstd>) DEFINATIONS <definations>])
function(add_shared_super_library TARGET_NAME)
    ...
    set(multiValueArgs SOURCES LIBS MANUAL_LIBS INCLUDES COMPILE_OPTIONS LINKER_OPTIONS DEFINATIONS)
    ...
    if(ARG_CSTD AND ARG_CPPSTD)
        message(FATAL_ERROR "Please specify either 'CSTD' or 'CPPSTD' but not both.")
    endif()

    if(ARG_NOPREFIX)
        set(SHARED_TARGET ${TARGET_NAME})
    else()
        set(SHARED_TARGET lib${TARGET_NAME})
    endif()

    add_library(${SHARED_TARGET} SHARED ${ARG_SOURCES})
    set_target_properties(${SHARED_TARGET} PROPERTIES OUTPUT_NAME ${TARGET_NAME})
    set_target_properties(${SHARED_TARGET} PROPERTIES POSITION_INDEPENDENT_CODE ON)
    if(ARG_NOPREFIX)
        set_target_properties(${SHARED_TARGET} PROPERTIES PREFIX "")
    endif()
    ...
    if(ARG_INCLUDES)
        target_include_directories(${SHARED_TARGET} PRIVATE ${ARG_INCLUDES})
    endif()
    if(ARG_COMPILE_OPTIONS)
        target_compile_options(${SHARED_TARGET} PRIVATE ${ARG_COMPILE_OPTIONS})
    endif()
    if(ARG_LINKER_OPTIONS)
        target_link_options(${SHARED_TARGET} PRIVATE ${ARG_LINKER_OPTIONS})
    endif()
    if(ARG_DEFINATIONS)
        target_compile_definitions(${SHARED_TARGET} PRIVATE ${ARG_DEFINATIONS})
    endif()

    if(ARG_LIBS)
        foreach(LIB ${ARG_LIBS})
            if(TARGET "lib${LIB}_shared" OR TARGET "${LIB}_shared")
    ...
    if(ARG_CSTD)
        target_compile_options(${SHARED_TARGET} PRIVATE "-std=c${ARG_CSTD}")
    endif()
    if(ARG_CPPSTD)
        target_compile_options(${SHARED_TARGET} PRIVATE "-std=c++${ARG_CPPSTD}")
    endif()
endfunction()

# Usage: add_static_super_library(<name> <NO_INTERFACE | USE_INTERFACE | USE_INTERFACE_NOLIBS | USE_INTERFACE_NOLIBS_NOFLAGS> <source(s)> [LIBS <libs> MANUAL_LIBS <libs> INCLUDES <includes> COMPILE_OPTIONS <options> (CPPSTD <cppstd> OR CSTD <cstd>) DEFINATIONS <definations>])
function(add_static_super_library TARGET_NAME)
    set(options DISABLE NOPREFIX NO_INTERFACE USE_INTERFACE USE_INTERFACE_NOLIBS USE_INTERFACE_NOLIBS_NOFLAGS SUPER_LIBS)
    set(oneValueArgs CPPSTD)
    set(multiValueArgs SOURCES LIBS MANUAL_LIBS INCLUDES COMPILE_OPTIONS DEFINATIONS)
    cmake_parse_arguments(ARG "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
    ...
    if(ARG_NOPREFIX)
        set_target_properties(${STATIC_TARGET} PROPERTIES PREFIX "")
    endif()

    if(ARG_USE_INTERFACE)
        target_link_libraries(${STATIC_TARGET} PRIVATE pmt::interface::shared)
    elseif(ARG_USE_INTERFACE_NOLIBS)
    ...
    if(ARG_INCLUDES)
        target_include_directories(${STATIC_TARGET} PRIVATE ${ARG_INCLUDES})
    endif()
    if(ARG_COMPILE_OPTIONS)
        target_compile_options(${STATIC_TARGET} PRIVATE ${ARG_COMPILE_OPTIONS})
    endif()
    if(ARG_DEFINATIONS)
        target_compile_definitions(${STATIC_TARGET} PRIVATE ${ARG_DEFINATIONS})
    endif()

    if(ARG_LIBS)
        foreach(LIB ${ARG_LIBS})
            if(TARGET "lib${LIB}_static" OR TARGET "${LIB}_static")
    ...
    if(ARG_CSTD)
        target_compile_options(${STATIC_TARGET} PRIVATE "-std=c${ARG_CSTD}")
    endif()
    if(ARG_CPPSTD)
        target_compile_options(${STATIC_TARGET} PRIVATE "-std=c++${ARG_CPPSTD}")
    endif()
endfunction()
...
# Usage: add_super_library(<name> <source(s)> [INCLUDES <includes> COMPILE_OPTIONS <options> LINKER_OPTIONS <linker-options> LINKER_SCRIPT <linker-script> DEFINATIONS <definations>])
...
function(add_include_directories dir)
    if(NOT EXISTS "${dir}")
        message(WARNING "Include directory does not exist: ${dir}")
        return()
    endif()
    get_property(current GLOBAL PROPERTY PMT_INCLUDE_DIRECTORIES)
    list(APPEND current "${dir}")
    set_property(GLOBAL PROPERTY PMT_INCLUDE_DIRECTORIES "${current}")
endfunction()
